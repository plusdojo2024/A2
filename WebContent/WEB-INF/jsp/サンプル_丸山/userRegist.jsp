...
<head>
	...
	<title>ユーザー登録 | 名刺管理</title>
</head>
<body id="top">
	<div  class="bg userRegist">
		<!--ヘッダー-->
		<header>
			<div class="title">
			...
			<nav class="nav">
				<ul>
                    ...
                    <li><a href="/myBC/LogoutServlet">ログイン</a></li>
				</ul>
			</nav>
		</header>
		<!--ヘッダーここまで-->
		...
		<!-- 時計ここまで -->
		<h1 class="theme">ユーザー登録</h1>
		<!--メイン-->
		<main>
			<div>
				<form id="userRegist_form" action="/myBC/UserRegistServlet" method="post">
					<table class="input">
						<tr>
						...
						<tr>
							...
							<td><input type="password" name="pw" class="userRegistPw" placeholder="パスワード" id="pw"></td><td><span id="eye" class="fa-solid fa-eye-slash" onclick="pushHideButton()"></span></td>
						</tr>
						<tr>
							<th class="loginForm"><label for="userName">ユーザー名</label></th>
							<td><input type="text" name="userName" placeholder="例）山田太郎" id="userName"></td>
						</tr>
						<tr>
							<td><input type="submit"  name="regist" value="REGIST" class="button"> </td>
	...
	<script>
		...
		let terget = document.getElementById('pw');
		...
		formObj.onsubmit = function(){
			if(formObj.id.value ==='' || formObj.pw.value ==='' || formObj.userName.value ===''){
				showAlert('ID、パスワード、ユーザー名のいずれかが未入力です！');
				return false;
			}
		}
